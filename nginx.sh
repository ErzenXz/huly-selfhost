...
# Always (re)generate container Nginx config used by docker compose
...
server {
    ...
    # Back-compat alias: some builds call /_account (singular)
    location /_account {
      proxy_http_version 1.1;
      proxy_set_header Upgrade $http_upgrade;
      proxy_set_header Connection "upgrade";
    ...
    # Proxy app-specific service paths through account (single service)
    location /_gmail {
      proxy_http_version 1.1;
      proxy_set_header Upgrade $http_upgrade;
      proxy_set_header Connection "upgrade";
    ...
    # File upload/download handled by front (auth + streaming to storage)
    location = /files {
      proxy_http_version 1.1;
      proxy_set_header Upgrade $http_upgrade;
      proxy_set_header Connection "upgrade";
      ...
      proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
      proxy_set_header X-Forwarded-Proto $scheme;
      proxy_pass http://front:8080/files;
    }
    location /files/ {
      proxy_http_version 1.1;
      proxy_set_header Upgrade $http_upgrade;
